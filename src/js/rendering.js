...
function draw3DWalls(x, y, width, height, cornerRadius, room) {
    ...
    // Define the wall edges
    const edges = [
        { name: 'top', startX: centerX - halfWidth + cornerRadius, startY: centerY - halfHeight, endX: centerX + halfWidth - cornerRadius, endY: centerY - halfHeight },
        { name: 'right', startX: centerX + halfWidth, startY: centerY - halfHeight + cornerRadius, endX: centerX + halfWidth, endY: centerY + halfHeight - cornerRadius },
        { name: 'bottom', startX: centerX + halfWidth - cornerRadius, startY: centerY + halfHeight, endX: centerX - halfWidth + cornerRadius, endY: centerY + halfHeight },
        { name: 'left', startX: centerX - halfWidth, startY: centerY + halfHeight - cornerRadius, endX: centerX - halfWidth, endY: centerY - halfHeight + cornerRadius }
    ];
    
    // Define the corners
    const corners = [
        { x: centerX - halfWidth + cornerRadius, y: centerY - halfHeight + cornerRadius, startAngle: Math.PI, endAngle: Math.PI * 1.5 },     // Top-left
        { x: centerX + halfWidth - cornerRadius, y: centerY - halfHeight + cornerRadius, startAngle: Math.PI * 1.5, endAngle: Math.PI * 2 }, // Top-right  
        { x: centerX + halfWidth - cornerRadius, y: centerY + halfHeight - cornerRadius, startAngle: 0, endAngle: Math.PI * 0.5 },           // Bottom-right
        { x: centerX - halfWidth - cornerRadius, y: centerY + halfHeight - cornerRadius, startAngle: Math.PI * 0.5, endAngle: Math.PI }      // Bottom-left
    ];
    
    // Helper function to get portal cuts on a specific edge
    function getPortalCutsOnEdge(edge) {
        const cuts = [];
        const buffer = 5;
        ...
        for (const portal of portals) {
            ...
            if (edge.name === 'top' && Math.abs(portal.y - edge.startY) < thickness + 10) {
                // Portal is on top edge
                if (portalLeft >= edge.startX && portalRight <= edge.endX) {
                    cutStart = portalLeft;
                    cutEnd = portalRight;
                }
            } else if (edge.name === 'bottom' && Math.abs(portal.y - edge.startY) < thickness + 10) {
                // Portal is on bottom edge
                if (portalRight >= edge.endX && portalLeft <= edge.startX) {
                    cutStart = portalRight;
                    cutEnd = portalLeft;
                }
            } else if (edge.name === 'right' && Math.abs(portal.x - edge.startX) < thickness + 10) {
                // Portal is on right edge
                if (portalTop >= edge.startY && portalBottom <= edge.endY) {
                    cutStart = portalTop;
                    cutEnd = portalBottom;
                }
            } else if (edge.name === 'left' && Math.abs(portal.x - edge.startX) < thickness + 10) {
                // Portal is on left edge
                if (portalBottom >= edge.endY && portalTop <= edge.startY) {
                    cutStart = portalBottom;
                    cutEnd = portalTop;
                }
            }
    ...
    function drawWallSegment(p1Outer, p2Outer, p1Inner, p2Inner) {
        // Project floor points
        const outerFloor1 = project3DPoint(p1Outer[0], p1Outer[1], 0, cameraX, cameraY);
        const outerFloor2 = project3DPoint(p2Outer[0], p2Outer[1], 0, cameraX, cameraY);
        const innerFloor1 = project3DPoint(p1Inner[0], p1Inner[1], 0, cameraX, cameraY);
        const innerFloor2 = project3DPoint(p2Inner[0], p2Inner[1], 0, cameraX, cameraY);
        
        // Project wall top points
        const outerTop1 = project3DPoint(p1Outer[0], p1Outer[1], PERSPECTIVE_CONFIG.wallHeight, cameraX, cameraY);
        const outerTop2 = project3DPoint(p2Outer[0], p2Outer[1], PERSPECTIVE_CONFIG.wallHeight, cameraX, cameraY);
        const innerTop1 = project3DPoint(p1Inner[0], p1Inner[1], PERSPECTIVE_CONFIG.wallHeight, cameraX, cameraY);
        const innerTop2 = project3DPoint(p2Inner[0], p2Inner[1], PERSPECTIVE_CONFIG.wallHeight, cameraX, cameraY);
        
        // Calculate wall normal for outer face
        const wallNormalX = -(p2Outer[1] - p1Outer[1]);
        const wallNormalY = p2Outer[0] - p1Outer[0];
        const cameraDirX = cameraX - (p1Outer[0] + p2Outer[0]) / 2;
        const cameraDirY = cameraY - (p1Outer[1] + p2Outer[1]) / 2;
        ...
        const innerNormalX = -(p2Inner[1] - p1Inner[1]);
        const innerNormalY = p2Inner[0] - p1Inner[0];
        const innerCameraDirX = cameraX - (p1Inner[0] + p2Inner[0]) / 2;
        const innerCameraDirY = cameraY - (p1Inner[1] + p2Inner[1]) / 2;
    ...
    // Draw each edge with portal cuts
    edges.forEach((edge, edgeIndex) => {
        const cuts = getPortalCutsOnEdge(edge);
        
        if (cuts.length === 0) {
            // No portals on this edge, draw the entire edge
            let p1Outer, p2Outer, p1Inner, p2Inner;
            
            if (edge.name === 'top') {
                p1Outer = [edge.startX, edge.startY - thickness];
                p2Outer = [edge.endX, edge.endY - thickness];
                p1Inner = [edge.startX, edge.startY];
                p2Inner = [edge.endX, edge.endY];
                ...
                p1Outer = [edge.startX + thickness, edge.startY];
                p2Outer = [edge.endX + thickness, edge.endY];
                p1Inner = [edge.startX, edge.startY];
                p2Inner = [edge.endX, edge.endY];
            ...
            drawWallSegment(p1Outer, p2Outer, p1Inner, p2Inner);
        } else {
            // Draw wall segments around portal cuts
            let currentPos = 0;
            
            for (const cut of cuts) {
                // Draw segment before the cut
                let p1Outer, p2Outer, p1Inner, p2Inner;
                
                if (edge.name === 'top') {
                    const segStartX = edge.startX + currentPos * (edge.endX - edge.startX) / Math.abs(edge.endX - edge.startX);
                    const segEndX = cut.start;
                    
                    p1Outer = [segStartX, edge.startY - thickness];
                    p2Outer = [segEndX, edge.endY - thickness];
                    p1Inner = [segStartX, edge.startY];
                    p2Inner = [segEndX, edge.endY];
                    
                    currentPos = cut.end - edge.startX;
                } else if (edge.name === 'bottom') {
                    const segStartX = edge.startX - currentPos * (edge.startX - edge.endX) / Math.abs(edge.startX - edge.endX);
                    const segEndX = cut.start;
                    ...
                    currentPos = edge.startX - cut.end;
                } else if (edge.name === 'right') {
                    const segStartY = edge.startY + currentPos * (edge.endY - edge.startY) / Math.abs(edge.endY - edge.startY);
                    const segEndY = cut.start;
                    
                    p1Outer = [edge.startX + thickness, segStartY];
                    p2Outer = [edge.endX + thickness, segEndY];
                    p1Inner = [edge.startX, segStartY];
                    p2Inner = [edge.endX, segEndY];
                    
                    currentPos = cut.end - edge.startY;
                } else if (edge.name === 'left') {
                    const segStartY = edge.startY - currentPos * (edge.startY - edge.endY) / Math.abs(edge.startY - edge.endY);
                    const segEndY = cut.start;
                    ...
                    currentPos = edge.startY - cut.end;
                }
            ...
            if (edge.name === 'top') {
                p1Outer = [lastCut.end, edge.startY - thickness];
                p2Outer = [edge.endX, edge.endY - thickness];
                p1Inner = [lastCut.end, edge.startY];
                p2Inner = [edge.endX, edge.endY];
            } else if (edge.name === 'bottom') {
                p1Outer = [lastCut.end, edge.startY + thickness];
                p2Outer = [edge.endX, edge.endY + thickness];
                ...
                p1Outer = [edge.startX + thickness, lastCut.end];
                p2Outer = [edge.endX + thickness, edge.endY];
                p1Inner = [edge.startX, lastCut.end];
                p2Inner = [edge.endX, edge.endY];
            } else if (edge.name === 'left') {
                p1Outer = [edge.startX - thickness, lastCut.end];
                p2Outer = [edge.endX - thickness, edge.endY];
        ...
        // Draw corner connecting to next edge
        const nextEdgeIndex = (edgeIndex + 1) % 4;
        const corner = corners[edgeIndex];
        const segmentsPerCorner = 20;
        
        for (let i = 0; i < segmentsPerCorner; i++) {
            const t1 = i / segmentsPerCorner;
            const t2 = (i + 1) / segmentsPerCorner;
            const angle1 = corner.startAngle + (corner.endAngle - corner.startAngle) * t1;
            const angle2 = corner.startAngle + (corner.endAngle - corner.startAngle) * t2;
            
            const outerRadius = cornerRadius + thickness;
            const innerRadius = cornerRadius;
            
            const p1Outer = [corner.x + outerRadius * Math.cos(angle1), corner.y + outerRadius * Math.sin(angle1)];
            const p2Outer = [corner.x + outerRadius * Math.cos(angle2), corner.y + outerRadius * Math.sin(angle2)];
            const p1Inner = [corner.x + innerRadius * Math.cos(angle1), corner.y + innerRadius * Math.sin(angle1)];
            const p2Inner = [corner.x + innerRadius * Math.cos(angle2), corner.y + innerRadius * Math.sin(angle2)];
